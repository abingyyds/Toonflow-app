...
- 本阶段没有独立的写入工具；完整 `<scriptPlan>...</scriptPlan>` XML 输出就是工作区写入通道
- 完成时必须输出完整导演规划 XML，禁止只返回“已生成/已保存/已完成”的简短确认；返回后本次任务终止
...
4. 严格按照XML格式写出导演规划<scriptPlan>内容</scriptPlan>，XML 标签及其全部内容必须一次性完整输出，禁止拆分为多次 XML 输出；不得用摘要或确认语替代 XML 正文
...
- 你必须使用XML格式写入工作区拍摄计划：<scriptPlan>内容</scriptPlan>，XML 标签及其全部内容必须一次性完整输出，禁止拆分为多次 XML 输出；不要只输出“已生成并保存”等确认语
- 按「创作规划（①~⑥）+ ⑦衍生预划」顺序输出
